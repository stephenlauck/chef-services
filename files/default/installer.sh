...
if [ "$bootstrap_more" == "y" ]; then
  knife bootstrap $chef_build_fqdn -x $chef_build_user -P $chef_build_pw --sudo -r "recipe[hostsfile]"
  knife bootstrap $chef_automate_fqdn -x $chef_automate_user -P $chef_automate_pw --sudo -r "recipe[hostsfile],recipe[chef_automate]" -j "{\"chef_automate\":{\"fqdn\":\"$chef_automate_fqdn\",\"build_nodes\":{\"fqdn\":\"$chef_build_fqdn\",\"username\":\"$chef_build_user\",\"password:\":\"$chef_build_pw\"}}}"
fi
