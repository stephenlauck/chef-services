%w(chef-server manage push-jobs-server).each do |pkg|
  remote_file "#{node['chef_server']['install_dir']}/#{pkg}-#{node[pkg]['version']}.#{::File.extname(node[pkg]['package_url'])}" do
...
EOS
  addons manage: { version: '2.4.3', config: '', package_source: "#{node['chef_server']['install_dir']}/manage-#{node['manage']['version']}.#{::File.extname(node['manage']['package_url'])}" },
         :"push-jobs-server" => { version: '2.1.0', config: '', package_source: "#{node['chef_server']['install_dir']}/push-jobs-server-#{node['push-jobs-server']['version']}.#{::File.extname(node['push-jobs-server']['package_url'])}" }
  accept_license true
